...
flang::FResult flang::DictionaryObject::exists(DictionaryObject &dict, flang::Interpreter &visitor, const std::shared_ptr<FunctionCallNode> &fn_node) {
...
flang::FResult flang::DictionaryObject::get(DictionaryObject &dict, flang::Interpreter &visitor, const std::shared_ptr<FunctionCallNode> &fn_node) {
    if (const auto res = verifyArgsCount(fn_node->args_node.args.size(), 1, dict.tok); res.has_value()) {
        return FResult::createError(
            RunTimeError,
            res.value(),
...
flang::FResult flang::DictionaryObject::keys(DictionaryObject &dict, flang::Interpreter &visitor, const std::shared_ptr<FunctionCallNode> &fn_node) {
...
flang::FResult flang::DictionaryObject::values(DictionaryObject &dict, flang::Interpreter &visitor, const std::shared_ptr<FunctionCallNode> &fn_node) {
    if (const auto res = verifyArgsCount(fn_node->args_node.args.size(), 0, dict.tok); res.has_value()) {
        return FResult::createError(
                RunTimeError,
                res.value(),
